...
export default {
    name: 'AppResults',
    props: ['messageInfo', 'resultsArray', 'posterImg', 'resultType'],
    methods: {
        searchFlag(language) {
...
<template>
    ...
    <div class="row mb-3 g-3">
        <div class="col-3" v-for="result in resultsArray" v-if="resultsArray.length > 0">
            <div class="card h-100">
                <img :src="[posterImg + result.poster_path]" class="imgPoster card-img-top" alt="..."
                ...
                <div class="card-body">
                    <h5 class="card-title">
                        <span v-if="resultType == 'movie'">
                            {{ result.title }} ({{ result.original_title }})
                        </span>
                        <span v-else-if="resultType == 'series'">
                            {{ result.name }} ({{ result.original_name }})
                        </span>
                    </h5>
                    <p class="card-text">
                    <ul class="list-group list-group-flush">
                        <li class="list-group-item" v-if="this.searchFlag(result.original_language)">
                            Lingua: <img :src="this.searchFlag(result.original_language)" height="20" alt="">
                        </li>
