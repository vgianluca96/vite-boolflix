...
<template>
    <div class="resultCard col-6 col-md-4 col-xl-3 col-xxl-2">
        ...
        <div class="resultDetails p-1">
            <div v-if="resultType == 'movie'">
                <strong>Titolo</strong>: {{ result.title }} <br>
                <strong>Titolo originale</strong>: {{ result.original_title }}
            </div>
            <div v-else-if="resultType == 'series'">
                <strong>Titolo</strong>: {{ result.name }} <br>
                <strong>Titolo originale</strong>: {{ result.original_name }}
            </div>
            <div v-if="this.searchFlag(result.original_language)">
                <strong>Lingua</strong>: <img :src="this.searchFlag(result.original_language)" height="20" alt="">
            </div>
            <div v-else>
                <strong>Lingua</strong>: {{ result.original_language }}
            </div>
            <div>
                <strong>Voto</strong>:
                <span v-for="n in 5">
